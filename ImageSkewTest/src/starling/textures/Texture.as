...
public class Texture {
	...
	/** Creates a texture from the compressed ATF format.
	*  Beware: you must not dispose 'data' if Starling should handle a lost device context. */
	public static function fromAtfDataAsync(data : ByteArray, scale : Number = 1, callBack : Function = null) : void {
		var context : Context3D = Starling.context;
		if (context == null)
			throw new MissingContextError();
		...
		uploadAtfDataAsync(nativeTexture, data);

		nativeTexture.addEventListener(Event.TEXTURE_READY, function(e : Event) : void {
			var concreteTexture : ConcreteTexture = new ConcreteTexture(nativeTexture, atfData.format,
				atfData.width, atfData.height, atfData.numTextures > 1, false, false, scale);
	...
	/** Uploads ATF data from a ByteArray to a native texture. */
	internal static function uploadAtfDataAsync(nativeTexture : flash.display3D.textures.Texture,
		data : ByteArray, offset : int = 0) : void {
